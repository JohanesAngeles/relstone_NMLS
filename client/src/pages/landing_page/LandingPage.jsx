  ...
  <div style={{ display: 'flex', gap: 2 }}>
    {[1,2,3,4,5].map(i => (
      <svg key={i} width="16" height="16" viewBox="0 0 24 24"
        fill={i <= rating ? '#F59E0B' : 'none'} stroke="#F59E0B" strokeWidth="1">
...
// ─── Course Carousel ───────────────────────────────────────────────────────────
const CourseCarousel = ({ courses, onEnroll, user, navigate }) => {
  ...
  const goTo = (newIndex, dir) => {
    ...
    setDirection(dir);
    setAnimating(true);
    setTimeout(() => { setIndex(clamped); setAnimating(false); }, 280);
  };

  const prev = () => goTo(index - 1, 'prev');
  const next = () => goTo(index + 1, 'next');

  const visible = courses.slice(index, index + perPage);

  const typeColor  = t => t === 'PE' ? '#2EABFE' : '#00B4B4';
  const typeBg     = t => t === 'PE' ? 'rgba(46,171,254,0.10)' : 'rgba(0,180,180,0.10)';
  const typeBorder = t => t === 'PE' ? 'rgba(46,171,254,0.30)' : 'rgba(0,180,180,0.30)';
  ...
  return (
    <div>
      ...
      }} className="lp-course-carousel-grid">
        {visible.map((c, i) => (
          <div key={c._id || i} className="lp-course-card">
            <div className="lp-course-header">
              <div style={{
                display: 'inline-flex', alignItems: 'center', gap: 6,
                padding: '3px 10px', borderRadius: 4, marginBottom: 8,
                background: typeBg(c.type), border: `0.5px solid ${typeBorder(c.type)}`,
                position: 'relative', zIndex: 1,
              }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: typeColor(c.type), textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  {c.type === 'PE' ? 'Pre-Licensing' : 'Continuing Ed'} · {c.type}
                </span>
              </div>
              <h3 className="lp-course-title">{c.title}</h3>
              <div className="lp-course-pills">
                <span className="lp-cpill lp-cpill--blue">NMLS Approved</span>
                {c.credit_hours && <span className="lp-cpill lp-cpill--orange">{c.credit_hours} hrs</span>}
                {Array.isArray(c.states_approved) && c.states_approved.length > 0 && (
                  <span className="lp-cpill lp-cpill--green">{c.states_approved[0]}</span>
                )}
              </div>
            </div>
            <div className="lp-course-body">
              <div className="lp-course-hrs">
                <span className="lp-hrs-num">{c.credit_hours || '—'}</span>
                <div className="lp-hrs-lbl"><div>NMLS CREDIT</div><div>HOURS</div></div>
              </div>
              {c.description && (
                <p className="lp-course-desc" style={{ display:'-webkit-box', WebkitLineClamp:3, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                  {c.description}
                </p>
              )}
              {Array.isArray(c.modules) && c.modules.length > 0 && (
                <ul className="lp-course-bullets">
                  {c.modules.slice(0, 4).map((m, mi) => (
                    <li key={mi}>
                  ...
                  {c.modules.length > 4 && (
                    <li style={{ color:'rgba(11,18,32,0.35)', fontStyle:'italic', listStyle:'none' }}>
                      +{c.modules.length - 4} more modules
                    </li>
                  )}
                </ul>
              )}
              <div className="lp-course-footer" style={{ marginTop: 'auto' }}>
                <span className="lp-course-price">
                  ${Number(c.price || 0).toFixed(2)}
                  <span className="lp-course-unit">/course</span>
                </span>
                <button className="lp-btn-blue" onClick={() => user ? navigate('/courses') : onEnroll()}>
      ...
      {/* Controls — only if more than 3 courses */}
      {total > perPage && (
        <>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:16, marginTop:32 }}>
            <button onClick={prev} disabled={index === 0} className="lp-carousel-arrow"
            ...
            <div style={{ display:'flex', gap:7, alignItems:'center' }}>
              {Array.from({ length: maxIndex + 1 }).map((_, i) => (
                <button key={i} onClick={() => goTo(i, i > index ? 'next' : 'prev')} style={{
                  width: i === index ? 24 : 7, height: 7, borderRadius: 99,
                  border: 'none', padding: 0, cursor: 'pointer',
                  background: i === index ? '#2EABFE' : 'rgba(46,171,254,0.25)',
                  transition: 'all .25s ease',
                }} />
              ))}
            </div>

            <button onClick={next} disabled={index >= maxIndex} className="lp-carousel-arrow"
              style={{ opacity: index >= maxIndex ? 0.35 : 1, cursor: index >= maxIndex ? 'default' : 'pointer' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
          ...
          <div style={{ textAlign:'center', marginTop:10, fontSize:12, fontWeight:600, color:'rgba(9,25,37,0.40)', fontFamily:"'Poppins',sans-serif" }}>
            Showing {index + 1}–{Math.min(index + perPage, total)} of {total} courses
          </div>
        </>
      )}
    </div>
...
const TestimonialCarousel = ({ testimonials }) => {
  ...
  const goTo = (newPage, dir) => {
    ...
    setDirection(dir);
    setAnimating(true);
    setTimeout(() => { setPage(newPage); setAnimating(false); }, 300);
  };

  const prev = () => goTo(page === 0 ? totalPages - 1 : page - 1, 'prev');
  const next = () => goTo(page === totalPages - 1 ? 0 : page + 1, 'next');
  ...
  return (
    <div>
            ...
            <div key={t._id} className="lp-tcard">
              ...
              <div className="lp-tcard-author">
                ...
                <div>
                  ...
                  <div className="lp-tcard-role">{t.role || t.course_title || 'RELSTONE Student'}</div>
                </div>
              </div>
            </div>
          ))}
      ...
      {totalPages > 1 && (
        <>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:16, marginTop:32 }}>
            <button onClick={prev} className="lp-carousel-arrow">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6" /></svg>
            </button>
            <div style={{ display:'flex', gap:8, alignItems:'center' }}>
              {Array.from({ length: totalPages }).map((_, i) => (
                <button key={i} onClick={() => goTo(i, i > page ? 'next' : 'prev')} style={{
                  width: i === page ? 28 : 8, height: 8, borderRadius: 99,
                  border: 'none', padding: 0, cursor: 'pointer',
                  background: i === page ? '#2EABFE' : 'rgba(46,171,254,0.25)',
                  transition: 'all .25s ease',
                }} />
              ))}
            </div>
            <button onClick={next} className="lp-carousel-arrow">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6" /></svg>
            </button>
          </div>
          <div style={{ textAlign:'center', marginTop:12, fontSize:12, fontWeight:600, color:'rgba(9,25,37,0.40)', fontFamily:"'Poppins',sans-serif" }}>
...
const PublicTestimonialForm = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await API.post('/testimonials/public', {
        ...
        role: role.trim() || undefined, rating,
  ...
  return (
    <form onSubmit={handleSubmit} style={FS.form}>
      ...
      {error && <div style={FS.errorBanner}>{error}</div>}

      <div style={FS.row}>
        <div style={FS.fieldWrap}>
          <label style={FS.label}>Your Name *</label>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. James R."
            style={{ ...FS.input, borderColor: errors.name ? '#EF4444' : '#7FA8C4' }} />
          {errors.name && <div style={FS.fieldErr}>{errors.name}</div>}
        </div>
        <div style={FS.fieldWrap}>
          <label style={FS.label}>Email Address *</label>
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com"
            style={{ ...FS.input, borderColor: errors.email ? '#EF4444' : '#7FA8C4' }} />
          {errors.email
            ? <div style={FS.fieldErr}>{errors.email}</div>
            : <div style={{ fontSize:11, color:'rgba(9,25,37,0.40)', fontFamily:"'Poppins',sans-serif" }}>For verification only — never shown publicly.</div>}
        </div>
      </div>

      <div style={FS.row}>
        <div style={FS.fieldWrap}>
          <label style={FS.label}>Role / Location <span style={{ opacity:.6 }}>(optional)</span></label>
          <input value={role} onChange={e => setRole(e.target.value)} placeholder="e.g. MLO — California" style={FS.input} />
        </div>
        <div style={FS.fieldWrap}>
          <label style={FS.label}>Course Name <span style={{ opacity:.6 }}>(optional)</span></label>
          <input value={courseTitle} onChange={e => setCourseTitle(e.target.value)} placeholder="e.g. 20-Hour SAFE Pre-Licensing" style={FS.input} />
        </div>
      </div>
          ...
          {[1,2,3,4,5].map(s => (
            <button key={s} type="button"
              ...
              <svg width="30" height="30" viewBox="0 0 24 24"
                ...
                stroke={s <= activeRating ? '#F59E0B' : '#CBD5E1'} strokeWidth="1.5"
                style={{ transition:'all 0.15s', transform: s <= activeRating ? 'scale(1.15)' : 'scale(1)' }}>
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
              </svg>
            </button>
          ))}
          <span style={{ fontSize:13, fontWeight:700, color: rating > 0 ? '#F59E0B' : 'rgba(9,25,37,0.40)', fontFamily:"'Poppins',sans-serif" }}>
      ...
      <div style={FS.fieldWrap}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          {[true, false].map(val => (
            <button key={String(val)} type="button" onClick={() => setRecommend(val)} style={{
              padding:'8px 20px', borderRadius:8, cursor:'pointer',
              fontFamily:"'Poppins',sans-serif", fontWeight:700, fontSize:13,
              border: recommend === val ? '1.5px solid #2EABFE' : '1px solid #CBD5E1',
              background: recommend === val ? 'rgba(46,171,254,0.1)' : '#fff',
              color: recommend === val ? '#2EABFE' : '#64748B', transition:'all 0.15s',
            }}>
              {val ? '👍 Yes' : '👎 No'}
            </button>
          ))}
      ...
      <div style={FS.fieldWrap}>
        ...
        <textarea value={comment} onChange={e => setComment(e.target.value)}
          placeholder="Tell others about your experience with RELSTONE…"
          rows={4} maxLength={2000}
          style={{ ...FS.input, resize:'vertical', lineHeight:1.65, borderColor: errors.comment ? '#EF4444' : '#7FA8C4' }} />
        <div style={{ display:'flex', justifyContent:'space-between', marginTop:4 }}>
          ...
          <div style={{ fontSize:11, color:'rgba(9,25,37,0.40)', fontFamily:"'Poppins',sans-serif" }}>{comment.length}/2000</div>
        </div>
      </div>
      ...
      <button type="submit" disabled={saving} style={{
        ...
        color:'#091925', fontSize:15, fontWeight:800,
        cursor: saving ? 'not-allowed' : 'pointer',
        fontFamily:"'Poppins',sans-serif", transition:'all 0.2s',
      }}>
        {saving ? 'Submitting…' : 'Submit My Review →'}
      </button>
      <div style={{ fontSize:11, color:'rgba(9,25,37,0.40)', textAlign:'center', fontFamily:"'Poppins',sans-serif", marginTop:4 }}>
        Reviews are moderated. Your email is used for verification only and never displayed publicly.
      </div>
    </form>
  );
};
...
// ── Form styles ────────────────────────────────────────────────────────────────
const FS = {
  form:        { display:'flex', flexDirection:'column', gap:16, background:'#ffffff', border:'1px solid #2EABFE', borderRadius:16, padding:'28px 24px', boxShadow:'0 4px 20px rgba(0,0,0,0.1)' },
  formTitle:   { fontSize:20, fontWeight:800, color:'#091925', fontFamily:"'Poppins',sans-serif" },
  formSub:     { fontSize:13, color:'#5B7384', fontFamily:"'Poppins',sans-serif", marginTop:-8 },
  row:         { display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 },
  fieldWrap:   { display:'flex', flexDirection:'column', gap:6 },
  label:       { fontSize:12, fontWeight:700, color:'#091925', fontFamily:"'Poppins',sans-serif", textTransform:'uppercase', letterSpacing:'0.04em' },
  input:       { padding:'10px 14px', borderRadius:8, border:'1px solid #7FA8C4', background:'#f8fafc', color:'#091925', fontSize:13, fontFamily:"'Poppins',sans-serif", outline:'none', width:'100%', boxSizing:'border-box' },
  fieldErr:    { fontSize:11, fontWeight:700, color:'#EF4444', fontFamily:"'Poppins',sans-serif" },
  errorBanner: { padding:'10px 14px', borderRadius:8, background:'rgba(239,68,68,0.15)', border:'1px solid rgba(239,68,68,0.35)', color:'#EF4444', fontSize:13, fontFamily:"'Poppins',sans-serif" },
};
...
const LandingPage = () => {
  ...
  const [lpCoursesLoading, setLpCoursesLoading] = useState(true);

  useEffect(() => {
    API.get('/courses')
      .then(res => setLpCourses(Array.isArray(res.data) ? res.data : []))
      .catch(() => {})
  ...
  const handleFormSuccess = () => { setFormSuccess(true); setShowForm(false); };
              ...
              <div className="lp-hero-badges">
                {['NMLS-Approved','All 50 States','Established 1978'].map((b, i) => (
                  <span key={b} style={{ display:'contents' }}>
                    {i > 0 && <div className="lp-badge-sep" />}
            ...
            <div className="lp-hero-right">
              <div className="lp-hero-stats-card">
                ...
                <div className="lp-req-list">
                  {[['SAFE Act Pre-Licensing Education','20 hrs'],['Annual Continuing Education','8 hrs / yr'],['Federal Law & Ethics','Included'],['State-Specific Law Electives','50 states'],['Non-Traditional Lending Standards','Included']].map(([t,b]) => (
      ...
      <section className="lp-courses" id="courses">
        <div className="lp-container">
          ...
          <div style={{ textAlign:'center', marginTop:32 }}>
            <button
              onClick={() => user ? navigate('/courses') : setModal('register')}
              style={{
                display:'inline-flex', alignItems:'center', gap:8,
                padding:'12px 28px', borderRadius:8, border:'1.5px solid #2EABFE',
                background:'transparent', color:'#2EABFE',
                fontFamily:"'Poppins',sans-serif", fontWeight:700, fontSize:14,
                cursor:'pointer', transition:'all .2s',
              }}
              onMouseEnter={e => { e.currentTarget.style.background='#2EABFE'; e.currentTarget.style.color='#091925'; }}
              onMouseLeave={e => { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='#2EABFE'; }}
            >
              View All Courses →
            </button>
          </div>
        ...
        <div className="lp-container">
          ...
          {testimonialsLoading && (
            <div className="lp-tcard-loading">
              {[1,2,3].map(i => <div key={i} className="lp-tcard-skeleton" />)}
            </div>
          )}
          {!testimonialsLoading && testimonials.length === 0 && (
            <p className="lp-tcard-empty">No testimonials available yet — be the first to share your experience!</p>
          )}
          {!testimonialsLoading && testimonials.length > 0 && (
          ...
          <div style={{ marginTop:40 }}>
            {formSuccess ? (
              <div style={{ textAlign:'center', padding:'28px 24px', background:'rgba(46,171,254,0.08)', border:'1px solid rgba(46,171,254,0.25)', borderRadius:14 }}>
                ...
                <div style={{ fontSize:18, fontWeight:800, color:'#091925', fontFamily:"'Poppins',sans-serif", marginBottom:8 }}>Thank you for your review!</div>
                <div style={{ fontSize:14, color:'rgba(9,25,37,0.55)', fontFamily:"'Poppins',sans-serif" }}>Your testimonial is pending approval and will appear here once reviewed by our team.</div>
              </div>
            ) : showForm ? (
...
/* ════ HERO WRAPPER ════ */
.lp-hero-wrapper { position:relative; background-color:#091925; background-size:cover; background-position:center top; background-repeat:no-repeat; padding-top:71px; }
.lp-hero-base { position:absolute; inset:0; background:rgba(9,25,37,0.28); z-index:1; pointer-events:none; }
.lp-hero-gradient { position:absolute; inset:0; background:linear-gradient(180deg,rgba(9,25,37,0.0) 0%,rgba(46,171,254,0.12) 100%); z-index:2; pointer-events:none; }

/* ════ NAV ════ */
.lp-topbar { position:fixed; top:0; left:0; right:0; z-index:1000; background:rgba(9,25,37,0.4); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); height:80px; display:flex; align-items:center; border-bottom:1px solid rgba(255,255,255,0.08); transition:all 0.3s cubic-bezier(0.4,0,0.2,1); }
.lp-topbar--scrolled { background:rgba(9,25,37,0.95); height:65px; box-shadow:0 10px 30px rgba(0,0,0,0.3); border-bottom-color:rgba(46,171,254,0.2); }
.lp-topbar-inner { width:100%; max-width:1300px; margin:0 auto; padding:0 24px; display:flex; align-items:center; justify-content:space-between; gap:20px; }
...
.lp-nav-link { color:rgba(255,255,255,0.7); font-size:14px; font-weight:500; text-decoration:none; padding:8px 16px; border-radius:8px; transition:all 0.2s; }
.lp-nav-link:hover { color:#fff; background:rgba(255,255,255,0.05); }
.lp-nav-right { display:flex; align-items:center; gap:12px; }
.lp-nav-ghost { color:#fff; font-weight:600; font-size:13px; background:transparent; border:1px solid rgba(255,255,255,0.2); padding:8px 18px; border-radius:8px; cursor:pointer; transition:0.2s; }
.lp-nav-ghost:hover { border-color:#fff; background:rgba(255,255,255,0.05); }
.lp-nav-enroll { background:#2EABFE; color:#091925; font-weight:700; font-size:13px; border:none; padding:9px 20px; border-radius:8px; cursor:pointer; transition:0.2s; }
...
@media (max-width:640px) {
  ...
  .lp-topbar { height:70px; } .lp-brand-logo { height:28px; }
  .lp-nav-right { gap:8px; } .lp-nav-enroll { padding:8px 14px; }
}

/* ════ HERO ════ */
.lp-hero { position:relative; background:transparent; display:flex; flex-direction:column; min-height:calc(100vh - 71px); justify-content:center; }
.lp-hero-inner { position:relative; z-index:3; flex:1; display:grid; grid-template-columns:1fr 460px; gap:40px; align-items:center; padding:50px 40px 44px; max-width:1200px; margin:0 auto; width:100%; }
.lp-hero-eyebrow-wrap { display:inline-flex; align-items:center; gap:8px; padding:0 12px; height:28px; background:rgba(46,171,254,0.1); border:0.5px solid #2EABFE; border-radius:100px; margin-bottom:18px; }
.lp-eyebrow-dot { width:6px; height:6px; border-radius:50%; background:#00FF09; box-shadow:0 0 4px #00FF09; flex-shrink:0; animation:blink 2s infinite; }
@keyframes blink { 0%,100%{opacity:1} 50%{opacity:.35} }
.lp-hero-eyebrow-text { font-size:12px; font-weight:700; color:#2EABFE; letter-spacing:0.5px; text-transform:uppercase; }
.lp-hero-h1 { font-size:58px; line-height:56px; font-weight:700; color:#fff; text-transform:uppercase; margin-bottom:18px; }
.lp-hero-accent { color:#2EABFE; }
.lp-hero-desc { font-size:15px; line-height:22px; font-weight:500; color:rgba(255,255,255,0.82); max-width:480px; margin-bottom:26px; }
.lp-hero-actions { display:flex; gap:10px; margin-bottom:18px; flex-wrap:wrap; }
.lp-btn-hero-solid { display:inline-flex; align-items:center; gap:8px; width:160px; height:44px; font-size:13px; font-weight:700; color:#091925; background:#2EABFE; border-radius:5px; border:0.5px solid #2EABFE; cursor:pointer; justify-content:center; transition:background .15s; }
.lp-btn-hero-solid:hover { background:#60C3FF; }
.lp-btn-hero-outline { display:inline-flex; align-items:center; justify-content:center; width:160px; height:44px; font-size:13px; font-weight:700; color:#fff; border:1px solid #fff; border-radius:5px; background:transparent; cursor:pointer; transition:all .15s; }
.lp-btn-hero-outline:hover { background:rgba(255,255,255,0.08); }
.lp-hero-badges { display:flex; align-items:center; flex-wrap:wrap; }
.lp-badge-item { display:flex; align-items:center; gap:5px; font-family:'JetBrains Mono',monospace; font-size:12px; font-weight:500; color:#fff; }
.lp-badge-sep { width:0; height:9px; border-left:0.5px solid #2EABFE; margin:0 12px; }
.lp-hero-stats-card { background:rgba(46,171,254,0.1); border:0.5px solid #2EABFE; border-radius:8px; padding:20px; }
.lp-stats-label { font-size:13px; font-weight:500; color:#fff; letter-spacing:0.5px; text-transform:uppercase; margin-bottom:12px; }
.lp-stats-grid { display:grid; grid-template-columns:1fr 1fr; gap:0; }
.lp-stat-box { background:rgba(46,171,254,0.1); border:0.5px solid #2EABFE; padding:10px; text-align:center; }
.lp-stat-num { font-family:'JetBrains Mono',monospace; font-size:28px; font-weight:800; color:#2EABFE; line-height:1.2; }
.lp-stat-desc { font-family:'JetBrains Mono',monospace; font-size:9px; font-weight:500; color:#fff; line-height:1.4; text-align:center; }
.lp-stats-divider { width:100%; height:0.5px; background:#2EABFE; margin:14px 0; }
.lp-req-label { font-size:13px; font-weight:500; color:#fff; letter-spacing:0.5px; text-transform:uppercase; margin-bottom:10px; }
...
.lp-statsbar-inner { display:grid; grid-template-columns:repeat(4,1fr); padding:16px 40px; max-width:1200px; margin:0 auto; position:relative; }
...
.lp-sbar-lbl { font-size:12px; font-weight:700; color:#fff; margin-top:3px; text-transform:uppercase; letter-spacing:0.5px; }

/* ════ Shared ════ */
.lp-eyebrow-small { font-size:14px; font-weight:500; color:#2EABFE; margin-bottom:6px; text-transform:uppercase; display:flex; align-items:center; gap:6px; }
.lp-eyebrow-blue  { font-size:14px; font-weight:500; color:#2EABFE; margin-bottom:6px; text-transform:uppercase; display:inline-block; }
.lp-eyebrow-sky   { font-size:14px; font-weight:500; color:#2EABFE; margin-bottom:6px; text-transform:uppercase; display:inline-block; }
.lp-h2 { font-size:38px; font-weight:700; line-height:40px; color:#091925; margin-bottom:12px; text-transform:uppercase; }
.lp-h2--light { color:#fff; }
.lp-sub { font-size:14px; font-weight:400; color:#091925; line-height:22px; }
.lp-sub--light { color:rgba(255,255,255,0.7); }
.lp-body { font-size:14px; font-weight:400; line-height:22px; color:#091925; margin-bottom:12px; }
...
.lp-btn-dark { display:inline-flex; align-items:center; padding:0 20px; height:44px; font-size:13px; font-weight:700; color:#fff; background:#091925; border-radius:5px; border:0.5px solid #091925; cursor:pointer; margin-top:8px; transition:background .15s; }
...
.lp-about-card-title { font-size:14px; font-weight:700; color:#091925; margin-bottom:3px; line-height:18px; }
.lp-about-card-desc  { font-size:13px; font-weight:400; color:#091925; line-height:18px; }
...
.lp-course-carousel-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; }
...
.lp-course-title { font-size:18px; font-weight:700; color:#fff; line-height:1.2; margin-bottom:8px; text-transform:capitalize; position:relative; z-index:1; }
.lp-course-pills { display:flex; gap:5px; flex-wrap:wrap; position:relative; z-index:1; }
.lp-cpill { padding:2px 8px; border-radius:4px; font-size:10px; font-weight:700; height:22px; display:inline-flex; align-items:center; text-transform:capitalize; }
...
.lp-cpill--teal   { background:rgba(0,180,180,0.1); color:#00B4B4; border:0.5px solid rgba(0,180,180,0.3); }
...
.lp-hrs-lbl { font-size:12px; font-weight:500; color:#7FA8C4; line-height:1.4; text-transform:uppercase; }
.lp-course-desc { font-size:13px; font-weight:400; line-height:20px; color:#091925; margin-bottom:12px; }
.lp-course-bullets { list-style:none; display:flex; flex-direction:column; gap:5px; flex:1; margin-bottom:16px; padding:0; }
...
.lp-course-unit { font-size:12px; font-weight:500; color:#7FA8C4; margin-left:2px; }
.lp-btn-blue { padding:0 14px; height:36px; font-size:13px; font-weight:700; color:#091925; background:#2EABFE; border:0.5px solid #2EABFE; border-radius:5px; cursor:pointer; transition:background .15s; white-space:nowrap; min-width:110px; font-family:'Poppins',sans-serif; }
.lp-btn-blue:hover { background:#1a9ee0; color:#fff; }

/* ── Carousel arrows ── */
.lp-carousel-arrow { width:42px; height:42px; border-radius:50%; border:1.5px solid rgba(46,171,254,0.35); background:transparent; cursor:pointer; display:grid; place-items:center; color:#2EABFE; transition:all .2s; }
.lp-carousel-arrow:hover { background:#2EABFE; color:#091925; border-color:#2EABFE; }
...
.lp-feat-title { font-size:15px; font-weight:700; color:#fff; margin-bottom:8px; line-height:22px; }
.lp-feat-desc  { font-size:13px; font-weight:400; line-height:20px; color:rgba(255,255,255,0.65); }
...
.lp-req-title { font-size:14px; font-weight:700; color:#091925; margin-bottom:4px; line-height:18px; }
.lp-req-desc  { font-size:12px; font-weight:400; line-height:16px; color:#091925; }
...
.lp-tcard-text   { font-size:13px; font-weight:400; line-height:18px; color:#091925; font-style:italic; flex:1; }
.lp-tcard-author { display:flex; align-items:center; gap:10px; }
.lp-tcard-avatar { width:36px; height:36px; border-radius:50%; background:#2EABFE; display:flex; align-items:center; justify-content:center; font-size:11px; font-weight:700; color:#091925; flex-shrink:0; }
.lp-tcard-name   { font-size:13px; font-weight:700; color:#091925; line-height:16px; }
.lp-tcard-role   { font-size:12px; font-weight:400; color:#5B7384; }
.lp-tcard-loading { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; }
.lp-tcard-skeleton { height:200px; border-radius:5px; background:linear-gradient(90deg,#e8edf0 25%,#d0dae0 50%,#e8edf0 75%); background-size:200% 100%; animation:shimmer 1.4s infinite; }
@keyframes shimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} }
.lp-tcard-empty { text-align:center; font-size:14px; color:#5B7384; padding:40px 0; }
...
.lp-faq-q { width:100%; padding:15px 18px; display:flex; justify-content:space-between; align-items:center; gap:14px; background:none; border:none; cursor:pointer; font-size:14px; font-weight:700; color:#091925; text-align:left; line-height:18px; font-family:'Poppins',sans-serif; }
...
.lp-faq-a { padding:12px 18px 16px; font-size:14px; font-weight:400; line-height:20px; color:#091925; background:rgba(46,171,254,0.05); font-family:'Poppins',sans-serif; }
...
.lp-cta-h2 { font-size:38px; font-weight:700; line-height:40px; color:#091925; margin-bottom:12px; text-transform:uppercase; }
...
.lp-signup-label { font-size:14px; font-weight:500; color:#2EABFE; margin-bottom:16px; text-transform:uppercase; display:block; }
...
.lp-footer-band-eye  { font-size:14px; font-weight:500; color:#2EABFE; margin-bottom:8px; display:flex; align-items:center; gap:6px; }
.lp-footer-band-h2   { font-size:40px; font-weight:700; line-height:42px; color:#fff; margin-bottom:12px; text-transform:uppercase; }
.lp-footer-band-sub  { font-size:14px; font-weight:400; line-height:22px; color:rgba(255,255,255,0.65); max-width:480px; }
...
.lp-footer-copy      { font-size:11px; color:rgba(255,255,255,0.42); }
.lp-footer-link-blue { color:#2EABFE; text-decoration:none; }
.lp-footer-links     { display:flex; align-items:center; gap:8px; }
.lp-footer-link      { font-size:11px; color:rgba(255,255,255,0.42); text-decoration:none; transition:color .15s; }
...
@media (max-width:900px) {
  ...
  .lp-course-carousel-grid { grid-template-columns:1fr 1fr !important; }
...
@media (max-width:640px) {
  ...
  .lp-course-carousel-grid { grid-template-columns:1fr !important; }
