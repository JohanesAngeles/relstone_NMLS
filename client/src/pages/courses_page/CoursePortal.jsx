...
import {
  ...
  AlertCircle, Star, X, FileText, ExternalLink,
} from "lucide-react";
...
const CoursePortal = () => {
  ...
  return (
    <div style={S.page}>
      ...
      {error && (
        <div style={S.errorBanner}><AlertCircle size={14} /> {error}</div>
      )}
                    ...
                    <div style={S.sidebarItemLeft}>
                      ...
                      <div>
                        <div style={S.sidebarLabel}>
                          {typeLabel}
                          {item.type === "lesson" && item.pdf_url && (
                            <span style={{ marginLeft: 4, color: "rgba(239,68,68,0.8)" }}>📄</span>
                          )}
                        </div>
        ...
        <main style={S.main}>
          <div style={S.contentWrap}>
            ...
            {current?.type === "pdf_gate" && (
              <PDFGateView item={current} onComplete={goNext} onPrev={goPrev} />
            )}
...
const PDFViewer = ({ url }) => {
  ...
  if (!url) return (
    <div style={S.pdfPlaceholder}>
      ...
      <div style={{ fontWeight: 700, fontSize: 15, color: "rgba(10,22,40,0.6)", marginBottom: 6 }}>
        Course Material PDF
      </div>
      <div style={{ fontSize: 13, color: "rgba(10,22,40,0.40)", fontWeight: 500 }}>
    ...
    <div style={S.pdfWrap}>
      <div style={S.pdfToolbar}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 13, color: "rgba(10,22,40,0.75)" }}>
            Course Material
          </span>
        </div>
      ...
      {pdfError ? (
        <div style={S.pdfErrorBox}>
          ...
          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 6, color: "rgba(10,22,40,0.7)" }}>
  ...
  return (
    <div style={S.lessonWrap}>
      ...
      <div style={S.pdfGateBanner}>
        ...
        <span>
          Review this study material carefully before proceeding to the
      ...
      <div style={S.pdfGateConfirmRow}>
        ...
        <label
          htmlFor="pdf-confirm"
          style={{ fontSize: 14, fontWeight: 700, color: "rgba(10,22,40,0.80)", cursor: "pointer", lineHeight: 1.4 }}
        >
  ...
  return (
    <div style={S.lessonWrap}>
      <div style={S.typePill}>
        ...
        {item.credit_hours > 0 && (
          <span style={{ opacity: 0.7 }}>· {item.credit_hours} hr{item.credit_hours !== 1 ? "s" : ""}</span>
        )}
      </div>
      ...
      {item.pdf_url && item.sections?.length > 0 && (
        <div style={S.viewToggle}>
          <button
            style={{ ...S.toggleBtn, ...(pdfView ? S.toggleBtnActive : {}) }}
            onClick={() => setPdfView(true)} type="button"
          >
            <FileText size={13} /> PDF View
          </button>
          <button
            style={{ ...S.toggleBtn, ...(!pdfView ? S.toggleBtnActive : {}) }}
            onClick={() => setPdfView(false)} type="button"
          >
            <BookOpen size={13} /> Outline View
          </button>
        </div>
      )}
            ...
            <div style={S.lessonText}>
              ...
              <ul style={S.lessonUl}>
                {item.sections.map((s, i) => (
                  <li key={i} style={S.lessonLi}>{s}</li>
                ))}
              </ul>
        ...
        <div style={allCorrect ? S.scorePassed : S.scoreFailed}>
          ...
          <div style={S.scoreLabel}>
            {allCorrect
              ...
              : `${item.questions.length - correctCount} incorrect — review and try again.`}
          </div>
        </div>
      )}
                ...
                {q.options.map((opt, oi) => {
                  ...
                  return (
                    <button key={oi}
                      ...
                      onClick={() => handleSelect(q.id, oi)} type="button"
                    >
...
const QuizView = ({ item, onFinish, onPrev }) => {
  ...
  const doSubmit = () => {
    ...
    setScore(pct);
    setPassed(pct >= (item.passingScore || 70));
    setSubmitted(true);
  };
    ...
    <div style={S.checkWrap}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        {!submitted && (
          <div style={{
            ...S.timerBadge,
            background:   urgentTime ? "rgba(239,68,68,0.10)" : "rgba(46,171,254,0.10)",
            border:       `1px solid ${urgentTime ? "rgba(239,68,68,0.30)" : "rgba(46,171,254,0.25)"}`,
            color:        urgentTime ? "rgba(185,28,28,1)" : "var(--cp-blue)",
          }}>
            ⏱ {fmt(timeLeft)}
          </div>
        )}
      ...
      {!submitted && (
        <div style={{ background: "rgba(2,8,23,0.07)", borderRadius: 999, height: 6, overflow: "hidden" }}>
          <div style={{
            ...
            background: "linear-gradient(90deg,var(--cp-teal),var(--cp-blue))",
            width: `${(answered / item.questions.length) * 100}%`,
      ...
      <div style={S.navRow}>
        ...
        {!submitted
          ? <button style={{ ...S.nextBtn, ...(!allAnswered ? S.nextBtnDim : {}) }} onClick={doSubmit} disabled={!allAnswered} type="button">
              Submit Exam
            </button>
          : passed
    ...
    <div style={S.completionWrap}>
      <div style={S.completionCard}>
        ...
        <div style={S.completionActions}>
          ...
          <button style={S.completionSecondary} onClick={() => navigate("/courses")}   type="button">Browse More Courses</button>
        </div>
      </div>
    </div>
  </div>
...
:root {
  --cp-dark: #0a1628; --cp-blue: #2EABFE; --cp-teal: #00B4B4;
  --cp-bg: #f4f6fa; --cp-card: #ffffff; --cp-border: rgba(2,8,23,0.09);
}
