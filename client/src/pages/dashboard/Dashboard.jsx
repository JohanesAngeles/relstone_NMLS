...
import {
  ...
  CheckCircle, Award, MapPin, Hash, ChevronRight, Search,
} from "lucide-react";
          ...
          <div style={S.heroInner}>
            ...
            <div style={S.profileRow}>
              <div style={S.profileChip}>
                ...
                <span>NMLS ID: <strong>{profile?.nmls_id || "Not set"}</strong></span>
              </div>
              <div style={S.profileChip}>
              ...
              <div style={S.profileChip}>
                ...
                <span>Total Completions: <strong>{completions?.total || 0}</strong></span>
              </div>
            </div>
            ...
            <div style={S.kpiGrid}>
              ...
              <KpiCard icon={<Clock    size={20} />} title="Pending Courses"    value={pendingCount} caption="In progress" tone="amber" />
            </div>
          </div>
        </section>
          ...
          <div style={S.cardBody}>
              ...
              <div style={S.gridTwo}>
                <div style={S.panel}>
                  ...
                  {recentCompletions.length === 0 ? (
                    <EmptyState icon={<Award size={18} />} title="No completions yet"
                      subtitle="Once you complete a course, it will show here."
                      actionLabel="Browse courses" onAction={() => navigate("/courses")} />
                  ) : (
                ...
                <div style={S.panel}>
                  ...
                  <div style={S.quickActions}>
                    ...
                    <button style={S.actionCard} type="button" onClick={() => setActiveTab("orders")}>
                      ...
                      <div style={S.actionText}><div style={S.actionTitle}>Check orders</div><div style={S.actionSub}>Track payment and status</div></div>
                      <ChevronRight size={18} />
                    </button>
                  </div>
                </div>
            ...
            {activeTab === "transcript" && (
              <div>
                ...
                {filteredTranscript.length === 0 ? (
                  <EmptyState icon={<FileText size={18} />}
            ...
            {activeTab === "orders" && (
              <div>
                ...
                {orders.length === 0 ? (
                  <EmptyState icon={<Clock size={18} />} title="No orders yet"
                      ...
                      <div key={i} style={S.orderCard}>
                        <div style={S.orderTop}>
                          <div style={S.orderLeft}>
                            ...
                            <div style={S.orderMeta}><span style={{ display:"inline-flex", alignItems:"center", gap:6 }}><Clock size={14} />{order?.createdAt ? new Date(order.createdAt).toLocaleDateString() : "-"}</span></div>
                          </div>
      ...
      <div style={{ display:"grid", gap:6 }}>
        ...
        <div style={S.rowMeta}>
          ...
          {item?.certificate_url && (
            <a href={item.certificate_url} target="_blank" rel="noreferrer" className="rs-link" style={S.metaLink}>
...
const badgeStyle = (type) => {
  ...
  const base = { display:"inline-flex", alignItems:"center", padding:"6px 10px", borderRadius:999, fontSize:12, fontWeight:800, letterSpacing:"0.2px" };
  if (t==="PE"||t.includes("PRE"))  return { ...base, color:"var(--rs-blue)",      background:"rgba(46,171,254,0.12)", border:"1px solid rgba(46,171,254,0.22)" };
  if (t==="CE"||t.includes("CONT")) return { ...base, color:"rgba(0,140,140,1)",   background:"rgba(0,180,180,0.12)",  border:"1px solid rgba(0,180,180,0.20)"  };
...
const orderStatusStyle = (status) => {
  ...
  const base = { display:"inline-flex", alignItems:"center", padding:"6px 10px", borderRadius:999, fontSize:12, fontWeight:800, letterSpacing:"0.2px", textTransform:"capitalize" };
  if (s==="paid"||s==="completed"||s==="success") return { ...base, color:"rgba(0,140,140,1)",   background:"rgba(0,180,180,0.12)",  border:"1px solid rgba(0,180,180,0.20)"  };
...
:root {
  ...
  --rs-shadow:0 18px 48px rgba(2,8,23,0.10); --rs-ring:rgba(46,171,254,0.28);
}
...
.rs-spinner{width:36px;height:36px;border-radius:999px;border:3px solid rgba(2,8,23,0.12);border-top-color:var(--rs-blue);animation:spin 1s linear infinite;}
...
const S = {
  ...
  tabBtnActive:{ borderColor:"rgba(46,171,254,0.32)", boxShadow:"0 0 0 5px rgba(46,171,254,0.28)", color:"var(--rs-dark)" },
