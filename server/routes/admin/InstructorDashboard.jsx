...
import {
  ...
  ChevronRight, ChevronLeft, BarChart2, TrendingUp, Search,
  ...
  DollarSign, XCircle, CreditCard, Package, HelpCircle, Archive
} from "lucide-react";
...
/* ─── Confirm Course Archive Modal ───────────────────────────────────────── */
const ConfirmCourseToggleModal = ({ course, onConfirm, onCancel, loading }) => {
  const willArchive = (course?.active ?? course?.is_active) !== false;
  return (
    <>
      <div onClick={onCancel} style={{
        position: "fixed", inset: 0, zIndex: 300,
      ...
      <div style={{
        ...
        <div style={{
          width: 56, height: 56, borderRadius: 18, margin: "0 auto 18px",
          background: willArchive ? "rgba(239,68,68,0.08)" : "rgba(0,180,180,0.08)",
          border: `1px solid ${willArchive ? "rgba(239,68,68,0.18)" : "rgba(0,180,180,0.20)"}`,
          display: "flex", alignItems: "center", justifyContent: "center",
          color: willArchive ? "rgba(220,38,38,0.85)" : "var(--rs-teal)",
        }}>
          <Archive size={26} />
        </div>
        <div style={{ fontSize: 18, fontWeight: 950, color: "rgba(11,18,32,0.88)", marginBottom: 8 }}>
          {willArchive ? "Archive Course?" : "Restore Course?"}
        </div>
        <div style={{ fontSize: 13, fontWeight: 700, color: "rgba(11,18,32,0.52)", lineHeight: 1.6, marginBottom: 6 }}>
        ...
        <div style={{ fontSize: 13, fontWeight: 600, color: "rgba(11,18,32,0.52)", lineHeight: 1.6, marginBottom: 24 }}>
          {willArchive
            ? "This course will be hidden from the active courses list and students."
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={onConfirm} type="button" disabled={loading} style={{
            flex: 1, height: 44,
            background: willArchive ? "rgba(220,38,38,0.90)" : "rgba(0,180,180,0.90)",
            border: "none", borderRadius: 12, cursor: "pointer",
            fontSize: 14, fontWeight: 900, color: "#fff", fontFamily: "inherit",
          }}>
            {loading ? "Saving…" : willArchive ? "Yes, Archive" : "Yes, Restore"}
          </button>
        </div>
      </div>
    </>
...
const ActivityLogPanel = ({ logs, loading, onRefresh }) => {
  ...
  const totalLogPages = Math.ceil(filtered.length / LOGS_PER_PAGE);
    ...
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          onAction={() => { setActionFilter("all"); setLogSearch(""); setLogPage(1); }}
        />
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          ...
          {totalLogPages > 1 && (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 24, paddingTop: 16, borderTop: "1px solid rgba(2,8,23,0.06)" }}>
              <button
                type="button"
                onClick={() => setLogPage(p => Math.max(1, p - 1))}
                disabled={logPage === 1}
                style={{ ...S.ghostBtn, opacity: logPage === 1 ? 0.4 : 1, cursor: logPage === 1 ? "not-allowed" : "pointer" }}
              >
                <ChevronLeft size={15} /> Previous
              </button>
              <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(11,18,32,0.50)" }}>
              ...
              <button
                ...
                style={{ ...S.ghostBtn, opacity: logPage === totalLogPages ? 0.4 : 1, cursor: logPage === totalLogPages ? "not-allowed" : "pointer" }}
              >
...
const InstructorDashboard = () => {
  ...
  const [courseToggleTarget,  setCourseToggleTarget]  = useState(null);
  const [courseToggleLoading, setCourseToggleLoading] = useState(false);
...
const [orderFilter, setOrderFilter] = useState("all");
  ...
  /* ── Toggle course archive/active ─────────────────────────────── */
  const handleCourseToggleActive = async () => {
    if (!courseToggleTarget) return;
    setCourseToggleLoading(true);
    try {
      const fallbackStatus = !(courseToggleTarget.active ?? courseToggleTarget.is_active);
      const res = await API.put(`/courses/${courseToggleTarget._id}`, { ...courseToggleTarget, is_active: fallbackStatus });
      const newStatus = res.data?.is_active ?? res.data?.active ?? fallbackStatus;
      
      const update = (list) => list.map(c =>
        String(c._id) === String(courseToggleTarget._id) ? { ...c, active: newStatus, is_active: newStatus } : c
      );
      setAllCourses(prev => update(prev));
      setData(prev => prev ? { ...prev, courses: update(prev.courses || []) } : prev);
      const newLog = {
        _id:             Date.now(),
        action:          "edit_course",
        instructor_name: user?.name || "Instructor",
        course_title:    courseToggleTarget.title,
        details:         `Course ${newStatus ? "restored" : "archived"}`,
        timestamp:       new Date().toISOString(),
      };
      setActivityLogs(prev => [newLog, ...prev]);
      setCourseToggleTarget(null);
    } catch (err) { console.error("Toggle course failed:", err); } 
    finally { setCourseToggleLoading(false); }
  };
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (courseStatusFilter === "active") {
      list = list.filter(c => (c.active ?? c.is_active) !== false);
    } else if (courseStatusFilter === "archived") {
      list = list.filter(c => (c.active ?? c.is_active) === false);
    }
  ...
  return (
    <div style={S.page}>
      ...
      {courseToggleTarget && (
        <ConfirmCourseToggleModal
          course={courseToggleTarget}
          loading={courseToggleLoading}
          onConfirm={handleCourseToggleActive}
          onCancel={() => setCourseToggleTarget(null)}
        />
      )}
          ...
          <div style={S.heroInner}>
            <div style={S.heroTop}>
              ...
              <div style={S.heroActions}>
                ...
                <button style={{ ...S.heroBtn, ...S.heroBtnAlt }} type="button" onClick={() => { setActiveTab("students"); setStudentPage(1); }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                <div style={{ display: "flex", gap: 6 }}>
                  {[
                    ...
                    onClick={() => { setStatusFilter(f.key); setStudentPage(1); }}>
                      {f.label}
                ...
                <div style={S.searchWrap}>
                  <Search size={15} style={{ color: "rgba(9,25,37,0.45)" }} />
                  <input style={S.searchInput} value={q} onChange={e => { setQ(e.target.value); setStudentPage(1); }} placeholder="Search by name, email, course, NMLS ID…" />
                </div>
              </div>
            )}
            {activeTab === "courses" && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", width: "100%", justifyContent: "space-between" }}>
                <div style={{ display: "flex", gap: 6 }}>
                  {[
                    ...
                    <button key={f.key} type="button"
                      style={{
                        ...S.filterChip,
                        ...(courseStatusFilter === f.key ? S.filterChipActive : {}),
                      }}
                      onClick={() => { setCourseStatusFilter(f.key); setCoursePage(1); }}>
                      {f.label}
                    </button>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  <div style={S.searchWrap}>
                    <Search size={15} style={{ color: "rgba(9,25,37,0.45)" }} />
                    <input style={S.searchInput} value={courseSearch} onChange={e => { setCourseSearch(e.target.value); setCoursePage(1); }} placeholder="Search courses…" />
                  </div>
                  <select
              ...
              <div style={S.gridTwo}>
                <div style={S.panel}>
                  <div style={S.panelHead}>
                    ...
                    <button style={S.ghostBtn} type="button" onClick={() => { setActiveTab("courses"); setCoursePage(1); }}>
                  ...
                  {recentCourses.length === 0 ? (
                    ...
                    <div style={{ display: "grid", gap: 10 }}>
                      {recentCourses.map((c, i) => (
                        <CourseMiniRow key={i} course={c} onEdit={() => setEditingCourse(c)} onToggleActive={() => setCourseToggleTarget(c)} />
                      ))}
                    </div>
                  )}
                </div>
                <div style={S.panel}>
                  ...
                  <div style={S.quickActions}>
                    ...
                    <ActionCard icon={<BookOpen size={17} />}     title="All Courses"       sub={`${displayCourses.length} courses available`}  onClick={() => { setActiveTab("courses"); setCoursePage(1); }} />
                    <ActionCard icon={<Users size={17} />}         title="All Students"      sub={`${displayStudents.length} students enrolled`}  onClick={() => { setActiveTab("students"); setStudentPage(1); }} />
                    <ActionCard icon={<UserX size={17} />}         title="Inactive Students" sub={`${inactiveCount} student${inactiveCount !== 1 ? "s" : ""} deactivated`} onClick={() => { setActiveTab("students"); setStatusFilter("inactive"); setStudentPage(1); }} />
              ...
              <div>
                <div style={S.sectionHead}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button style={S.refreshBtn} type="button" onClick={() => { setAllCourses([]); setCourseSearch(""); setCoursePage(1); }}>
                      <RefreshCw size={13} /> Refresh
                    </button>
                  </div>
                </div>
                {coursesLoading ? (
                  ...
                  <EmptyState icon={<BookOpen size={18} />} title="No courses found" subtitle="Try a different search term." actionLabel="Clear search" onAction={() => { setCourseSearch(""); setCoursePage(1); }} />
                ) : (
                  <>
                    <div style={S.courseGrid}>
                      {paginatedCourses.map((c, i) => (
                        <CourseCard key={c._id || i} course={c} onEdit={() => setEditingCourse(c)} onToggleActive={() => setCourseToggleTarget(c)} />
                      ))}
                    </div>
                    {totalCoursePages > 1 && (
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 24, paddingTop: 16, borderTop: "1px solid rgba(2,8,23,0.06)" }}>
                        <button
                          ...
                          style={{ ...S.ghostBtn, opacity: coursePage === 1 ? 0.4 : 1, cursor: coursePage === 1 ? "not-allowed" : "pointer" }}
                        >
                          <ChevronLeft size={15} /> Previous
                        </button>
                        <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(11,18,32,0.50)" }}>
                        ...
                        <button
                          ...
                          style={{ ...S.ghostBtn, opacity: coursePage === totalCoursePages ? 0.4 : 1, cursor: coursePage === totalCoursePages ? "not-allowed" : "pointer" }}
                        >
                          Next 10 <ChevronRight size={15} />
                        </button>
                      </div>
            ...
            {activeTab === "students" && (
              <div>
                <div style={S.sectionHead}>
                  ...
                  <button style={S.refreshBtn} type="button" onClick={() => { setAllStudents([]); setQ(""); setStudentPage(1); }}>
                    <RefreshCw size={13} /> Refresh
                  </button>
                </div>
                {studentsLoading ? (
                  ...
                  <EmptyState
                    ...
                    onAction={() => { setQ(""); setStatusFilter("all"); setStudentPage(1); if (!q.trim() && statusFilter === "all") setAllStudents([]); }}
                  />
                ) : (
                  <>
                    ...
                    {totalStudentPages > 1 && (
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 24, paddingTop: 16, borderTop: "1px solid rgba(2,8,23,0.06)" }}>
                        <button
                          ...
                          style={{ ...S.ghostBtn, opacity: studentPage === 1 ? 0.4 : 1, cursor: studentPage === 1 ? "not-allowed" : "pointer" }}
                        >
                          <ChevronLeft size={15} /> Previous
                        </button>
                        <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(11,18,32,0.50)" }}>
                        ...
                        <button
                          ...
                          style={{ ...S.ghostBtn, opacity: studentPage === totalStudentPages ? 0.4 : 1, cursor: studentPage === totalStudentPages ? "not-allowed" : "pointer" }}
                        >
                          Next 10 <ChevronRight size={15} />
                        </button>
                      </div>
...
const CourseMiniRow = ({ course, onEdit, onToggleActive }) => {
  const navigate  = useNavigate();
  const type      = String(course?.type || "").toUpperCase();
  ...
  return (
    <div style={{ ...S.rowCard, cursor: "pointer" }}>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        ...
        <button type="button" onClick={e => { e.stopPropagation(); onToggleActive(); }} title={active ? "Archive course" : "Restore course"} style={{
          display: "flex", alignItems: "center", justifyContent: "center",
          width: 30, height: 30, borderRadius: 8, flexShrink: 0,
          border: active ? "1px solid rgba(239,68,68,0.30)" : "1px solid rgba(0,180,180,0.28)",
          background: active ? "rgba(239,68,68,0.06)" : "rgba(0,180,180,0.07)",
          cursor: "pointer", color: active ? "rgba(185,28,28,1)" : "#00B4B4",
        }}>
          <Archive size={14} />
        </button>
...
const CourseCard = ({ course, onEdit, onToggleActive }) => {
  const navigate  = useNavigate();
  const type      = String(course?.type || "").toUpperCase();
    ...
    <div style={{ ...S.courseCard, cursor: "pointer" }} className="rs-course-card"
      ...
      <div style={S.courseCardTop}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button type="button" title={active ? "Archive course" : "Restore course"} onClick={e => { e.stopPropagation(); onToggleActive(); }} style={{
            display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 8,
            border: active ? "1px solid rgba(239,68,68,0.30)" : "1px solid rgba(0,180,180,0.28)",
            background: active ? "rgba(239,68,68,0.06)" : "rgba(0,180,180,0.07)",
            cursor: "pointer", color: active ? "rgba(185,28,28,1)" : "#00B4B4", fontSize: 12, fontWeight: 800,
          }}>
            <Archive size={13} /> {active ? "Archive" : "Restore"}
          </button>
        </div>
      </div>
